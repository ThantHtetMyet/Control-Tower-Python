...
class RTUPMPDFGenerator:
    ...
    def _build_basic_info(self, report_data: dict, report_title: str):
        ...
        table = Table([[Paragraph(label, self.card_label_style), Paragraph(self._as_text(value), self.card_value_style)] for label, value in rows],
    ...
    def _build_summary_section(self, rtu_form: dict):
        ...
        return [Paragraph("RTU Maintenance Summary", self.section_header), table, Spacer(1, 18)]
    ...
    def _build_label_value_table(self, items):
        rows = [[Paragraph(label, self.card_label_style), Paragraph(self._as_text(value), self.card_value_style)] for label, value in items]
    ...
    def _build_image_gallery(self, title: str, images: list):
        gallery = []
        if not images:
            gallery.append(self._build_placeholder_box(f"No images uploaded for {title.lower()}."))
            gallery.append(Spacer(1, 8))
            return gallery

        gallery.append(Paragraph(title, self.subsection_header))
        cards = []
        for meta in self._prepare_image_metadata(images):
            img_flow = self._create_image_flowable(meta)
            if not img_flow:
                continue
        ...
        if not cards:
            gallery.append(self._build_placeholder_box("Unable to render uploaded images."))
            gallery.append(Spacer(1, 8))
            return gallery
        ...
        gallery_table = Table(rows, colWidths=[3.0 * inch, 3.0 * inch], hAlign="LEFT")
    ...
    def _format_datetime(self, date_value):
        ...
        if isinstance(date_value, dict):
            # attempt common keys or first value
            for key in ("value", "date", "Date", "timestamp"):
                if key in date_value:
                    date_value = date_value[key]
                    break
            else:
                # fallback to first value or string repr
                try:
                    date_value = next(iter(date_value.values()))
                except Exception:
